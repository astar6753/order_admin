...
$(function(){
    ...
    $("#ai_id").keyup(function(){
        if(isEmpty($(this).val())) {
            ...
            $("#ai_id~.not_valid").css("display","none");
        }
    })
    $("#ai_id").change(function(){
        $.ajax({
            ...
            success:function(r){
                ...
                if(r) {
                    $(".duplicate").html("이미 사용 중인 아이디입니다.")
                    $(".duplicate").css("display","block");
                }
                else {
                    $(".duplicate").html("사용가능한 아이디입니다.")
                    $(".duplicate").css("display","block");
                }
            }
        })
    ...
    $("#join").click(function(){
        ...
        if(!accept) return;
        // if(!dateValidate($("#year").val()+"-"+$("#month option:selected").val()+"-"+$("#date").val())) {alert("올바르지 않은 날짜값입니다."); return;}
